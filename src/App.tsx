...
import { generateMonthlyPlan, summarizeChainSpend, summarizeDailyCoverage } from "./lib/generator";
...
const budgetStatusLabel: Record<MonthlyPlan["budgetStatus"], string> = {
  dentro: "Dentro del presupuesto",
  ajustado: "Muy ajustado",
  excedido: "Por encima del presupuesto"
};

const preferenceOptions = ["mediterranea", "alto en proteina", "rapido", "vegetal"];
  ...
  return (
    <div className="app-shell">
      <header className="hero">
        <div>
          <p className="eyebrow">Generador mensual automatizado</p>
          <h1>Planificador nutricional saludable para Paiporta</h1>
          <p className="hero-copy">
            Genera menus mensuales para 30 o 31 dias con validacion de micronutrientes, recetas detalladas,
            disponibilidad local en Mercadona y Consum de Paiporta, y lista de compra organizada por secciones.
          </p>
        </div>
        <div className="hero-note">
          <strong>Metodologia</strong>
          <p>
            El motor usa una base local de alimentos con valores medios por 100 g y productos de supermercado
            configurados para Paiporta. Los precios y disponibilidades son mantenibles desde el catalogo interno.
          </p>
        </div>
      </header>
      ...
      <main className="layout">
          ...
          <div className="form-grid">
            <label>
              ...
              <input
                ...
                onChange={(event) =>
                  setFormState((current) => ({ ...current, people: Number(event.target.value) || 1 }))
                }
              />
            </label>
              ...
              <tbody>
                {plan.nutrientProgress.map((item) => (
                  <tr key={item.nutrient}>
                    ...
                    <td className={item.percentage >= 100 ? "success-text" : "warning-text"}>{formatNumber(item.percentage)} %</td>
                  </tr>
                ))}
              </tbody>
            </table>
            ...
            {currentDay.meals.map((meal) => (
              <article key={`${currentDay.day}-${meal.mealType}`} className="meal-card">
                ...
                <div className="meal-nutrients">
                  ...
                  <span>{formatNumber(meal.nutrients.protein)} g proteinas</span>
                  <span>{formatNumber(meal.nutrients.fiber)} g fibra</span>
                  <span>{formatNumber(meal.nutrients.calcium)} mg calcio</span>
                </div>

                <div className="note-list">
                  {meal.notes.map((note, index) => (
                    <p key={`${meal.recipe.id}-note-${index}`}>{note}</p>
                  ))}
                </div>
              </article>
            ))}
        ...
        <section className="panel">
          <div className="panel-header">
            <h2>Disponibilidad local del dia {currentDay.day}</h2>
          </div>
              ...
              <article key={food.id} className="availability-card">
                ...
                {food.supermarkets.map((store) => (
                  <div key={`${food.id}-${store.chain}`} className="store-box">
                    ...
                    <span>Precio: {formatNumber(store.price)} EUR por {store.packSizeGrams} g</span>
        ...
        <section className="panel">
          <div className="panel-header">
            <h2>Lista de la compra mensual</h2>
          </div>
              ...
              <article key={`${group.chain}-${group.section}`} className="shopping-group">
                ...
                <ul>
                  {group.items.map((item) => (
                    <li key={`${group.chain}-${item.foodId}`}>
                      <strong>{item.productName}</strong> · {formatNumber(item.gramsNeeded)} g · {item.packsNeeded} envases
                      · {formatNumber(item.estimatedCost)} EUR · Alternativas: {item.alternatives.join(", ") || "Sin alternativa"}
                    </li>
                  ))}
                </ul>
              </article>
